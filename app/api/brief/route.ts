...
export async function POST(req: Request) {
  ...
  const companyEmail = process.env.COMPANY_EMAIL ?? "hello@loechsar.com"
  ...
  const htmlContent = `
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0A0A0C;color:#EDEDED;border-radius:12px;overflow:hidden;">
      <div style="background:#C2A46D;padding:32px 40px;">
        <h1 style="margin:0;font-size:28px;font-weight:900;letter-spacing:-0.02em;color:#0B0B0C;">LO ECHSAR</h1>
        <p style="margin:8px 0 0;font-size:12px;color:rgba(110,112,116,0.8);letter-spacing:0.2em;text-transform:uppercase;">Outdoor Advertising Brief</p>
      </div>
      <div style="padding:40px;">
        <h2 style="color:#C2A46D;font-size:18px;margin:0 0 24px;">Brief Received</h2>
        ...
        <h3 style="color:var(--primary);font-size:14px;font-weight:700;text-transform:uppercase;letter-spacing:0.15em;margin:0 0 16px;">Selected Billboards</h3>
        <div style="background:rgba(194,164,109,0.15);border:1px solid rgba(194,164,109,0.2);border-radius:8px;padding:20px;margin-bottom:32px;">
        ...
        ${brief ? `<h3 style="color:#C2A46D;font-size:14px;font-weight:700;text-transform:uppercase;letter-spacing:0.15em;margin:0 0 12px;">Brief Details</h3><p style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:20px;font-size:14px;line-height:1.7;color:rgba(243,241,236,0.8);margin:0 0 32px;">${brief}</p>` : ""}
        <p style="font-size:12px;color:rgba(110,112,116,0.7);margin:0;">A LO ECHSAR strategist will contact you within 24 hours.</p>
      </div>
    </div>
  `
  ...
  try {
    ...
    await transporter.sendMail({
      from:    `"LO ECHSAR" <${process.env.SMTP_USER}>`,
      to:      email,
      subject: "Your LO ECHSAR Billboard Brief  Received",
      html:    htmlContent,
    })

    // Email to company
    await transporter.sendMail({
      from:    `"LO ECHSAR Brief System" <${process.env.SMTP_USER}>`,
      to:      companyEmail,
