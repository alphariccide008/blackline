...
// Badge colors: map known badges to inline styles so we can replace blue usages with gold where needed
const badgeColors: Record<string, { background: string; color: string; border: string }> = {
  "Top Pick":  { background: "rgba(194,164,109,0.15)", color: "var(--primary)", border: "1px solid rgba(194,164,109,0.25)" },
  "Digital":   { background: "rgba(128,90,213,0.12)", color: "#8b5cf6", border: "1px solid rgba(139,92,246,0.18)" },
  "Premium":   { background: "rgba(250,204,21,0.12)", color: "#f59e0b", border: "1px solid rgba(245,158,11,0.18)" },
  "Hot Deal":  { background: "rgba(239,68,68,0.12)", color: "#ef4444", border: "1px solid rgba(239,68,68,0.18)" },
  "New":       { background: "rgba(16,185,129,0.12)", color: "#10b981", border: "1px solid rgba(16,185,129,0.18)" },
  "Available": { background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.9)", border: "1px solid rgba(255,255,255,0.12)" },
}
    ...
    >
      ...
      <Link href={`/billboards/${b.id}`} className="block">
        <div className="relative aspect-[4/3] overflow-hidden">
          ...
          <div className="absolute top-3 left-3">
            {(() => {
              const c = badgeColors[b.badge] ?? badgeColors["Available"]
              return (
                <span className="text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full" style={{ background: c.background, color: c.color, border: c.border }}>
                  {b.badge}
                </span>
              )
            })()}
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="flex items-center gap-1 mb-4">
          <Zap className="h-3 w-3 flex-shrink-0" style={{ color: "var(--primary)" }} />
          <span className="text-[11px] font-bold" style={{ color: "var(--primary)" }}>{b.impressions} impressions</span>
        </div>
        <div className="mt-auto">
          <button
            ...
            style={{
              ...
              border: inCart ? "1px solid rgba(194,164,109,0.35)" : "1px solid transparent",
              color: "#fff",
            }}
          >
...
export function BillboardStrip() {
  ...
  return (
    <section className="py-24 relative overflow-hidden" style={{ background: "#111115" }}>
      {/* Subtle bg glow */}
      {/* Subtle bg glow (gold) */}
      <div className="absolute top-0 right-0 w-[500px] h-[500px] rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, rgba(194,164,109,0.08) 0%, transparent 70%)" }} />
      ...
      <div className="container-wide">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          ...
          >
            ...
            <h2 className="text-3xl md:text-5xl font-black text-white tracking-tight leading-tight">
              Premium Outdoor<br /><span style={{ color: "var(--primary)" }}>Locations.</span>
            </h2>
        ...
        <motion.div
          ...
          style={{ background: "rgba(194,164,109,0.08)", border: "1px solid rgba(194,164,109,0.15)" }}
        >
          <div>
